...
<template>
  ...
  <div v-if="detail" class="p-16">
    ...
    <van-cell-group :border="false" class="detail-cell--group mt-16">
      <van-cell title="时间" icon="clock-o" :label="detail.start_time" :border="false" />
      <van-cell title="地点" icon="location-o" :label="detail.address" center />
    </van-cell-group>
    ...
    <van-action-bar placeholder>
      ...
      <van-action-bar-button type="primary" text="立即报名" :to="{ name: 'QiyueEventSignup', query: { event_id: detail.id } }" />
    </van-action-bar>
  </div>
</template>
